...
describe("Solpp plugin", async function () {
    describe("js-config-project", async function () {
        helpers_1.useEnvironment("js-config-project");
        it("should evaluate symbols as javascript functions", async function () {
            ...
            const generatedContractA = fs_1.readFileSync(paths[0]).toString();
    ...
    describe("json-config-project", async function () {
        helpers_1.useEnvironment("json-config-project");
    ...
    describe("hardhat-project", async function () {
        ...
        it("should collapse empty lines", async function () {
            const contractPath = path_1.join(this.env.config.paths.sources, "B.sol");
            const content = fs_1.readFileSync(contractPath).toString();
            ...
            const generatedContent = fs_1.readFileSync(paths[0]).toString();
        ...
        describe.skip("fail-project", async function () {
            helpers_1.useEnvironment("fail-project");
            it("should fail when symbol does not exist", async function () {
                const contractPath = path_1.join(this.env.config.paths.sources, "A.sol");
                const content = fs_1.readFileSync(contractPath).toString();
                const files = [[contractPath, content]];
                const opts = {};
